...
app.get("/apinit/getModulesByTechnologyIdForBatch", async (req, res) => {
  try {
...
app.get("/validateStudentEmail", async (req, res) => {
  try {
...
app.get("/validateStudentPhone", async (req, res) => {
  try {
    console.log("Student Email Validate API");
    const { phoneNumber } = req.query;

    if (!phoneNumber) {
      return res.status(400).send({ error: "phoneNumber parameter is required" });
    }
...
app.post("/apinit/Create_StudentBatch", async (req, res) => {
  try {
...
app.post("/apinit/addStudentsBatch", async (req, res) => {
  try {
...
app.get("/apinit/fetchBatchesByMentorId", async (req, res) => {
  try {
...
app.get("/apinit/studentAssessmentData", async (req, res) => {
  try {
    const questionTypeId = req.params.questionTypeId;
...
// Angular-Vlabs API
app.post("/api/vlabs/insertstudents", async (req, res) => {
