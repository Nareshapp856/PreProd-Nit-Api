...
exports.assignCurriculumToFaculty = async (req, res) => {
  try {
    const { uid, assignedBatchesId, curriculumId } = req.body;
    ...
    const { recordset } = await inputQuery(
      _query,
      ["uid", "assignedBatchesId", "curriculumId"],
      [sqlTypes.int, sqlTypes.int, sqlTypes.uniqueidentifier],
      [uid, assignedBatchesId, curriculumId]
    );
...
exports.updateAssignedCurriculum = async (req, res) => {
  try {
    const { assignedBatchesId, id, curriculumId, time, startDate } = req.body;
    ...
    const query = `EXEC USP_UpdatedAssingedCurriculum @ID=@id, @AssignedBatchesId=@assignedBatchesId, @CurriculumID=@curriculumId, @StartDate=@startDate, @Time=@time`;

    const result = await inputQuery(
      query,
      ["id", "assignedBatchesId", "curriculumId", "time", "startDate"],
      [
        sqlTypes.int,
        sqlTypes.int,
