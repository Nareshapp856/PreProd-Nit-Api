...
async function inputQuery(
  ...
  try {
    const pool = await db.getConnection(); // Ensure db.getConnection is implemented correctly
    const request = pool.request();

    inputs.forEach((input, index) => {
      if (types[index] === "Structured") {
        request.input(input, db.sql.TVP, parameters[index]); // Use sql.TVP for table-valued parameters
      } else if (db.sql[types[index]]) {
        request.input(input, db.sql[types[index]], parameters[index]);
      } else {
        console.error(`Invalid SQL type: ${types[index]}`);
