...
function SideNav() {
  ...
  const { myPerformance } = useFeatureFlags();
  ...
  const myPerformanceNav = () => {
    navigate("/my-performance");
  };

  const weekCalenderNav = () => {
    navigate("/week-calender");
  };
  const menuDashboard = () => {
    navigate("/");
    toggleMenu();
  ...
  return !isSmScreen ? (
    ...
    >
      <div className="ms-2 mt-3 mb-3">
        <img src={brand_logo} alt="NareshIT" width={240} />
      </div>
      ...
      <MenuList>
        ...
        <MenuItem
          ...
          onClick={weekCalenderNav}
        >
          <List sx={{ paddingBlock: 0, marginBlock: 0.4 }} className="w-full">
            <ListItem
              style={{
                backgroundColor:
                  location.pathname === "/week-calender"
                    ? "rgba(30,64,175,.20)"
                    : null,
              }}
              className="rounded"
