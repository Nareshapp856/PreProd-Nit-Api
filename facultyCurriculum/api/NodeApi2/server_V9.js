...
app.post("/api/nit_v1/AuthenticateStudent", async (req, res) => {
  try {
    ...
    if (
      ...
      const message = result.output.Message;
      console.log("response message in 2:", message);
      res.status(401).send(message);
    } else {
...
app.put("/apinit/updateAssessmentdetails", async (req, res) => {
  try {
    console.log("Harish", req.body.data);
    await sql.connect(sqlConfig);
    const {
    ...
    const result =
      await sql.query`EXEC [dbo].[USP_UpdatedAssingedCurriculum] ${testDescription}, ${testStartDate}, ${testEndDate}, ${testStartTime},  ${testEndTime}, ${status},${AssessmentID},${BatchId}`;
    console.log(result);
    res.status(200).send("Test information updated successfully.");
  } catch (err) {
    console.error(err);
...
// Api to Check whether CourseCurriculum Name already exist or not.
